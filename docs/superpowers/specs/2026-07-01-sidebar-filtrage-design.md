...
**Date** : 2026-07-01
**Périmètre** : feature `corpus` (bibliothèque / recherche). Écran uniquement = `Sidebar.tsx` + `CorpusFilters.tsx` ; aucun changement serveur requis.
...
- `src/components/Sidebar.tsx` — layout 3 zones (corps défilant + pied fixe), bouton Réinitialiser permanent (`disabled` piloté par `hasActiveFilters`) et crédit Lulumineuse.
- `src/components/CorpusFilters.tsx` — toggle mots-clés toujours visible.
