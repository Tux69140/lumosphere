...
| 7 | Historique — ingestion | **Import d'un fichier d'export Telegram Desktop (JSON)**, via un **script SSH exécuté par l'équipe dev** — opération **ponctuelle de mise en place**, **pas** une action de l'éditrice. Pas de page web, pas de compte Telethon sur le serveur mutualisé. L'éditrice ne fait que **réviser** les lots historiques qui en découlent. |
