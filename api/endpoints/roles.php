...
{
    return match (true) {
        $method === 'GET'    && $id !== null && $action === 'oeuvres'       =>
            dal_get_role_oeuvre_access($pdo, $ctx, $id),
        $method === 'PUT'    && $id !== null && $action === 'oeuvres'       => dal_set_role_oeuvre_access(
            $pdo,
            $ctx, $id, $body['oeuvre_ids'] ?? []
        ),
        $method === 'POST'   && $id === null                                => dal_create_role(
            $pdo,
            $ctx, $body['nom'] ?? '', $body['permission_ids'] ?? []
        ),
        ...
        $method === 'GET'    && $id !== null                                =>
            dal_get_role_with_permissions($pdo, $ctx, $id),
        $method === 'PUT'    && $id !== null && $action === 'permissions'   => dal_update_role_permissions(
            $pdo,
            $ctx, $id, $body['permission_ids'] ?? []
        ),
