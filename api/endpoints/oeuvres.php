...
function endpoint_oeuvres(PDO $pdo, array $ctx, string $method, ?int $id, ?array $body, ?string $action): array
{
    return match (true) {
        $method === 'GET'    && $id === null                         =>
            dal_find_oeuvres($pdo, $ctx, isset($_GET['auteur_id']) ? (int) $_GET['auteur_id'] : null),
        $method === 'GET'    && $id !== null                         => dal_get_oeuvre($pdo, $ctx, $id),
        $method === 'POST'   && $id === null                         => dal_create_oeuvre($pdo, $ctx, $body ?? []),
        $method === 'POST'   && $id !== null && $action === 'source' =>
            dal_link_oeuvre_source($pdo, $ctx, $id, isset($body['source_id']) ? (int) $body['source_id'] : null),
