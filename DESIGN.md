...
**Coquille centrée.** Le header reste pleine largeur ; en dessous, header + sidebar + contenu vivent dans un conteneur centré `max-w-screen-2xl mx-auto`, avec un espacement `gap-6` entre la sidebar et le `main` (`flex lg:flex-row`). Sur très grand écran, cette coquille laisse apparaître des marges symétriques plutôt que de remplir le viewport bord à bord — changement de philosophie assumé par rapport à la version précédente (« l'app remplit l'écran », sidebar collée au bord gauche).
